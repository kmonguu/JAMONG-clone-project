...
.board_list th { font-weight:bold; font-size:17px;border-top:1px solid #dddddd;border-bottom:1px solid #dddddd;background:#f8f8f8 }
.board_list th { white-space:nowrap; height:50px; overflow:hidden; text-align:center;color:#6e6e6e;font-size:17px; }
 
.board_list td {height:50px;border-bottom:1px solid #dddddd;color:#6e6e6e;font-size:17px}
...
.board_list .comment { font-family:Tahoma; font-size:16px; color:#EE5A00; }

.board_button { clear:both; padding:10px 0 0 0; display:inline-block; width:100%; }
.Boardpage { padding-bottom:0px; }
...
<!--게시판 서치부분-->
<style>
.board_search { float:right; margin-bottom:20px; }
.search_select { 
	width:125px; height:40px; border:1px solid #d8d8d8; position:relative; float:left; font-size:16px; padding-left:15px; background:#f9f9f9; outline:none; box-sizing:border-box; color:#5d5d5d; font-weight:400; 
...
.board_search #search_input > #stx { border:0; width:165px; height:38px; margin:0; padding:0 0 0 10px; letter-spacing:0px; color:#5d5d5d; font-size:16px; font-weight:400; }
...
<div style="margin:0 auto; width:<?=$width?>px;" >
    ...
    <form name="fboardlist" method="post">
		...
		<table cellspacing="0" cellpadding="0" class="board_list">
			<caption><?=$board["bo_subject"]?> 게시물 리스트 - 번호, 제목, 작성자, 작성일로 구성</caption>
			<colgroup>
				<col width="50" />
			...
			<tr>
				...
				<th scope="col" >제목</th>
				<th scope="col" >작성자</th>
				<th scope="col" ><!--<?=subject_sort_link('wr_datetime', $qstr2, 1)?>-->작성일<!--</a>--></th>
				<!-- <th scope="col" >조회</th> -->
			...
			?>
				...
				<tr>
					<td class="num" scope="row" >
						<?
						if ($list[$i][is_notice]) // 공지사항
							echo "<b><img src='$board_skin_path/img/noticeicon.png' alt='공지글' /></b>";
					...
					<td class="subject">
						...
						if ($list[$i][is_notice])
							echo "<a href='{$list[$i][href]}'><span class='notice'>".cut_str($list[$i]['wr_subject'], 55)."</span></a>";
						else
							echo "<a href='{$list[$i][href]}'>".cut_str($list[$i]['wr_subject'], 55)."</a>";
